...
;;(ql:quickload :kmrcl)			   ; => To load "kmrcl":

;; Copyrightの表示
(copyright "diary-system" "dev2.3")	; =>
;; (:PROGRAM-NAME "diary-system" :VERSION "dev2.3" :COPYRIGHT "2020" :AUTHOR
...
(setf cls-con (let ((s nil))
		(lambda () (push (read) s)))) ; => #<CLOSURE (LAMBDA ()) {1003B26A2B}>

;; (if (eq (funcall a) nil)
;;     (print "クロージャーaはnilです。")
;;   (print "クロージャーaはnilではないです。")) ; =>
;; (funcall a)				      ; => (NIL NIL)
;; (length a)				      ; =>
;; "クロージャーaはnilではないです。" "クロージャーaはnilではないです。"
;; "クロージャーaはnilではないです。" "クロージャーaはnilではないです。"
;;"クロージャーaはnilではないです。" "クロージャーaはnilではないです。"
;; T T
;; クロージャーaにシンボルを追加したけど、T Tになる。
;; つまりnilと同じという扱いになっているっぽい。
;; T T
;; a	       ; => #<CLOSURE (LAMBDA ()) {10021E874B}>
;; ()
;; ;;T T
;; (funcall a)    ; => (BANANA APPLE)
;;>(funcall a)
;;> (TEST2 TEST1)
...
;;  (defun no-paren (lst)
;;    (unless (null lst)
;;      (prin1 (string (car lst)))
;;      (no-paren (cdr lst))))		; => NO-PAREN

;; (string (no-paren l))			; => "HOGE""FOO""BAR""NIL"

;;  (setf l '(hoge foo bar))		; => (HOGE FOO BAR)
;; (mapcar #'prin1 l)			; => HOGEFOOBAR(HOGE FOO BAR)
 ;; (no-paren l)				; => HOGEFOOBARNIL
 ...
 ;;   (format *query-io* "~a: " prompt)
 ;;   (force-output *query-io*)
 ;;   (mkstr (funcall a)))			; => PROMPT-READ2
 ;;   (mkstr(car(funcall a))) だと単語と単語の間にスペースがある時に
 ;; 手前の単語しか表示しない事になる。つまりこの書き方だとダメ。[2020-08-01 09:50:01]
 ;; スペース以下のcdr部が落ちる問題あり[2020-07-31 10:44:27]
 ;; コンテンツのクロージャーの内容表示は出来た！
 ;; しかし、１行のみしか入力出来ない。
 ;; prompt-readをクロージャーで書き換えるか？[2020-07-30 02:52:42]
 ;; クロージャーの書き込み終了直前でクロージャーからファイル書き込みを
 ;; 入れてからクロージャー書き込みを終了させるという事は出来るか？[2020-07-31 09:27:59]

 ;; (defun prompt-read3 (prompt)
 ;;   "プロンプト入力情報の読み込みcontents用"
 ;;   (format *query-io* "~a: " prompt)
 ;;   (force-output *query-io*)
 ;;   (mkstr (mapcar (lambda (x) (prin1 x))
 ;; 		  (reverse (funcall a))))) ; => PROMPT-READ3

 ;; クロージャーの内容を括弧を外した状態で入力順で表示
 ;; (let ((cls-con2 (funcall cls-con)))
 ;;   (mapcar (lambda(x)(string x))
 ;; 	  (reverse cls-con2)))		; => ("TEST8" "TEST9" "TEST10" "TEST11" "TEST12" "TEST13" "TEST14" "TEST15" "TEST16")
...
;; 					; => (NIL NIL TEST5 TEST TEST4 TEST2 TEST2 NIL TEST2 TEST1)
;; (mkstr b)				; => "(NIL NIL TEST5 TEST TEST4 TEST2 TEST2 NIL TEST2 TEST1)"
