...
;; (defun signal-handler (signo signal-cb &key evnet-cb)) ; => SIGNAL-HANDLER
;; (signal-handler 2 (lambda (sig) (format t "got SIGINT: ~a~%" sig))
;; 		(lambda (err) (format t "error processing signal callback: ~a~%" err))) ; =>
;; (defun free-signal-handler (signo))	; => FREE-SIGNAL-HANDLER
;; (signal-handler 2
;;   (lambda (sig)
;;     (close-server *my-app-server*)
;;     (free-signal-handler 2)))		; => NIL
;; (defun clear-signal-handlers ())	; => CLEAR-SIGNAL-HANDLERS
...
;; write-to-stringを使ったらどうだろうか？
;; (mapcar #'write-to-string '(ab cd ef gh)) ; => ("AB" "CD" "EF" "GH")
;;(mapcar #'prin1-to-string '(ab cd ef gh)) ; => ("AB" "CD" "EF" "GH")
;; (mapcar #'string '(ab cd ef gh))	  ; => ("AB" "CD" "EF" "GH")
;; (mapcar #'write '(ab cd ef gh))	; => ABCDEFGH(AB CD EF GH)
;;  (mapcar #'pprint '(ab cd ef gh))	; =>
;; AB
;; CD
;; EF
;; GH(NIL NIL NIL NIL)
;;(edit-contents c1)			       ; =>
;; funcall は複数要素の引数に関数を渡す、applyはlistの要素に関数を渡す。
;; (funcall #'+ 1 2 3)			; => 6
;; (apply #'+ '(1 2 3))			; => 6
;; (defun run ()
;;   (loop
...
(defun run ()
   ...
   (when (null st1) (return)))
    ...
    (restart-case (error "Error!!!")
		  (finish-run ()
			      :report "finish-run"
			      :interactive (edit-contents (funcall cls-con3 nil))
			      (main))
		  (ignore ()
			  :report "Ignore!!"
			  nil)))	; => RUN
...
;;      (invoke-restart 'fuga-restart 9)
;;      (format t "~&③after invoke-restart~%")))) ; => TEST-RESTART-BIND
